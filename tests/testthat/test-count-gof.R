...
test_that("count GOF tests run on abun and recover ~1 dispersion when well-fit", {
  ...
  d <- tobs_test_dispersion(fit, n.samples = 100)
  z <- tobs_test_zero_inflation(fit, n.samples = 100)
  o <- tobs_test_outliers(fit, n.samples = 100)
...
test_that("count GOF tests run on removal / distance / dyn_abun", {
  ...
  fitr <- tobs(~ abund_cov1 + abund_cov2, data = simr$data, family = removal(K_max = 60),
               ...
               control = list(verbose = FALSE))
  expect_gof(tobs_test_dispersion(fitr, n.samples = 80))
  expect_gof(tobs_test_zero_inflation(fitr, n.samples = 80))
  ...
  fitd <- tobs(~ abund_cov1 + abund_cov2, data = simd$data,
               ...
               control = list(verbose = FALSE))
  expect_gof(tobs_test_dispersion(fitd, n.samples = 80))
  expect_gof(tobs_test_outliers(fitd, n.samples = 80))
  ...
  fitda <- tobs(~ abund_cov1, data = simda$data, family = dyn_abun(K_max = 35),
                ...
                control = list(verbose = FALSE))
  expect_gof(tobs_test_dispersion(fitda, n.samples = 50))
  expect_gof(tobs_test_zero_inflation(fitda, n.samples = 50))
})
