...
.batch_fused_identity_at <- function(N, J = 4L, offset_arm = NULL) {
  ...
  # Per-species equivalence (the fused path only reorganises the same math):
  # the grid result carries the independent kernel's fields, and every one --
  # modes, log-marginals, the per-grid inner-covariance precision Q (store_Q),
  # the solve-health fields -- agrees with it cell for cell.
  for (pair in list(list(s0, bat$per_species[[1]]),
                    list(s1, bat$per_species[[2]]))) {
    single <- pair[[1]]; bs <- pair[[2]]
    expect_identical(names(bs), names(single))
    expect_equal(bs, single, tolerance = 1e-9)
