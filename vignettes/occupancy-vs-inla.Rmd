...
| Coefficient summary | `m$summary.fixed` | `coef()`, `summary()`, `confint()` | one vector, `psi_` / `p_` prefixes |
...
answers the standard R generics. `coef()` gives one coefficient vector
with `psi_` and `p_` prefixes so the two arms never run together;
