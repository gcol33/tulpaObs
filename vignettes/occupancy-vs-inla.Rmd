...
| Multi-season / dynamic | none direct | `tobs(family = dyn_occu())` with `col_formula` / `ext_formula` | colonisation + extinction HMM |
...
| Shared field across arms | `copy = ...` in a second `f()` | `id = "u"` tag + `copy("u")` in the other formula | the occupancy / detection arms share one realisation |
...
| Model comparison | `control.compute = list(waic = TRUE)` | `tobs_waic()` | WAIC from the pointwise marginal log-lik |
...
site effect across both arms, tag it with `id` and `copy()` it into the
...
an inverse-link response curve with a credible band; and `tobs_waic()`
