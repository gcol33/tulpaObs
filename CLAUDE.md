...
| Joint occu + cover      | Yes     | —     | `occu_cover()`. **Non-spatial** (`method = "laplace"`, `R/occu_cover.R`): cell-level psi + per-visit detection + per-visit cover (beta or lognormal) on the exact two-state marginal. **Spatial default** (`method = "nested_laplace"`, `R/occu_cover_joint_coupled.R`): the `joint_coupled` engine routes through `tulpa_nested_laplace_joint()` with the `occu_cover_{lognormal,beta}` cell-coupling spec (gcol33/tulpa#32) — 3-arm joint nested-Laplace with outer-grid integration over `(sigma, alpha)` and a per-cell occupancy mixture's closed-form derivatives driving the inner Newton. Lognormal + beta positive arms both supported. ~150-300x faster than v3_nested at N=100 (measured `dev_notes/probe_bench_v3_vs_jc.R`) and reliably completes at N=200+ where v3 trips on a missing-value compare in its outer BFGS. Recovery: 10-seed lognormal + 10-seed beta (`test-occu-cover-joint-coupled.R`); status `"experimental"`. **Spatially-varying temporal trend** (gcol33/tulpaObs#15): `control = list(trend = list(weight = "<col>"))` adds a SECOND coupled ICAR field — a trend field weighted by a per-cell covariate — on top of the intercept field, via tulpa's multi-block copy path (`prior = list(icar_intercept, icar_trend)`, two `copy` specs onto the pos arm, per-block `svc_weight`). The intercept field (`fit$spatial_field`) couples uniformly; the trend field (`fit$trend_field`) is weighted by `weight_i` on the psi (per-cell) and pos (per-visit broadcast) arms with its own scale (`alpha_trend`, `control$alpha.grid.trend`). The p (detection) arm is excluded from BOTH fields by its `field_coef = 0` (one per-arm scalar zeroes the field on every block — `svc_weight = 0` on the p arm is NOT equivalent: it leaves the p-arm intercept biased through the cell-coupling cross-arm Hessian, so `field_coef = 0` is the correct exclusion; verified `dev_notes/_phase1_altwire.R`). Recovery: 10-seed 2-field lognormal (`test-occu-cover-trend.R`). A formula-DSL bridge via `svc()`/`copy()` terms is a deferred follow-up. **Escape hatches**: `control$engine = "v3_nested"` (the pure-R outer-BFGS path, `R/occu_cover_nested.R`, lognormal only — still gated by `test-occu-cover-spatial.R`), `control$engine = "v2_joint"` (v2's joint Laplace, kept for the (z, alpha, sigma) ridge investigation). |
