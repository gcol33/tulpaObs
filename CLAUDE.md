...
| Community distance spatial-factor (`ms_distance`) | n-L | — | `ms_distance()` + `icar()`/`car_proper()`/`bym2()`/`spde()` (`sfMsDS`, #117), optionally + `latent(n)`. A shared field REQUIRES `method="nested_laplace"` (`.dispatch_ms_distance` errors under `laplace`) — same gate as the sibling `ms_count`/`ms_abun` spatial-factor rows. Field on the ABUNDANCE arm only; a `spatial()`/`latent()` term on the detection formula errors. Working oracle = the SAME Louis formula as `ms_abun` — `score = grad_lam`, `curv = info_lam - var_N * swl^2` (both are count marginals with `B_i = diag(info) - var_N v v'`; only the kernel supplying the pieces differs). Oracle FD-validated (score 1.6e-9, curv = FD truncation, cor 1.000000). Poisson only; temporal/re/svc not wired. **The per-species Hessian IS assembled now (#161)**: it used to be left to the community EM's finite difference, at `2U` sweeps per species per Newton step with every sweep summing over the latent N, which made `test-ms-distance.R` the most expensive file in the recovery tier (18422.6s at tier 3, of which the two latent blocks are 98.8%; the cap on `full-recovery.yaml` is 350 min, so that ONE file was 88% of it). `.tobs_ms_distance_info_block()` builds the per-site Louis block `B_i = diag(info_lam, info_sig) - Var(N_i|y) v v'` from what `cpp_distance_site_sweep` already returns (`info_lam`, `info_sig_obs`, `var_N`, `swl`, `vN_sig`); distance has ONE unit per site so it is three `crossprod`s against diagonal weights, no per-site loop (the N-mixture needs one only because its visit count varies). Measured on the lfMsDS fixture: community EM **2.3x** faster with the fit UNCHANGED -- `mu` to 8.3e-14, `Sigma` to 3.3e-12, `b_s` to 2.3e-13, reported SEs to 3.4e-11 relative. **The sign inside `v` is NOT the sibling's**: `v = (-swl, -vN_sig)`, because the distance kernel stores `vN_d` already negated (`-p_k/(1-p)`) where `nmix_site_marginal()`'s `v` carries `+p`. That is invisible on the diagonal and wrong by ~2x on the cross block, so it is asserted against the finite difference rather than reasoned across (`test-ms-distance-info.R`, which also asserts the cross block is >10% of the diagonal so the agreement cannot be vacuous, and that the flipped convention is REJECTED). Half-normal only: under the hazard key `grad_b`/`info_b` come back already summed over sites and the per-site detection cross terms are not exported, so the shared log-shape global cannot be sandwiched and that key keeps the FD fallback |
...
R/
  ...
  engine_defaults.R         — `.TOBS_ENGINE_DEFAULTS` / `.TOBS_FAMILY_DEFAULTS` (#183): the per-engine SAMPLER defaults, one table. `.tobs_control_defaults(control, engine, family)` fills every knob the user left unset (resolve once per dispatcher branch); `.tobs_default(engine, knob, family)` reads one knob inline. Scope is the sampler knobs ONLY -- `max.iter`/`tol` are per-ROUTE Laplace-EM values (`ms_occu_cover()` iterates 30 at 1e-3 on its own EM but warm-starts its sampler with 200 at 1e-4; `ms_occu()`'s plain areal C++ EM uses 100 where its latent fitter uses 200), so they stay at their call sites. Log-link families (`ms_count`/`jsdm`/`ms_abun`) carry `sigma.beta=10` under nuts, logit-link occupancy families 5 -- matching the C++ model defaults; that split is a `.TOBS_FAMILY_DEFAULTS` row, NOT a uniform "community NUTS" value. `test-engine-defaults.R` pins every resolved profile.
    **Every sampler fitter reads it now (#188)** -- the table used to be read only by the 14 community dispatch branches while 29 fitters restated the knobs in their own formals. For everything routed through `.tobs_fit_model()` (occu/dyn_occu/int_occu + abun/removal/distance/fp_occu/dyn_abun/count) those formals were DEAD: that entry forwards explicit values, so `abun(method="nuts")` ran `.tobs_fit_model`'s `n.iter = 2000` and never its own `1000L` (measured: 2050 iterations, 2000 kept). `cover_nuts`/`occu_cover_nuts`/`occu_multiscale_cover_nuts` have their own dispatch, so THEIR formals were live and diverged. Now every sampler knob is a `NULL` formal + `.tobs_fill_sampler(environment(), engine, ...)` as the fitter's first statement; `test-engine-defaults.R` asserts structurally that no fitter carries a literal and that each calls the filler, so a new sampler family cannot reopen it.
    Divergences DELETED (proven artifacts): `n.iter` 2000 -> 1000 on occu/cover/occu_cover NUTS -- commit 8975470 changed `n.iter` from TOTAL to kept-post-warmup on exactly those paths and left the literal, so a default that had always kept 2000-1000=1000 silently began keeping 2000 (that commit's own message records the behaviour change); and occu `pg_gibbs` 2000/1000 -> the shared 3000/1500, which was keeping 1000 draws where every `ms_*` sibling kept 1500 for no recorded reason (verified: `test-occu-pg-gibbs{,-spatial}.R` 29/29 green after).
    Divergences RECORDED (`.TOBS_SINGLE_SPECIES_NUTS` / `_LAPLACE`, resolved by `.tobs_single_species_defaults(engine)`): the `.tobs_fit_model()` entry keeps `sigma.beta = 10` (vs the community 5, on laplace/nested_laplace too), `adapt.delta = 0.8` (vs 0.9) and `seed = 42` (vs 1). Each is the value the single-species recovery/coverage tests were calibrated against; the override belongs to the ENTRY, not to the nine families passing through it, hence one constant rather than nine family rows.
    **`n.iter` means the opposite thing under `pg_gibbs`**: TOTAL sweeps, warmup taken out of it (kept = `length(seq.int(n.warmup+1, n.iter, by=n.thin))` = 1500), where under `nuts` it is the kept count and the run is `n.iter + n.warmup`. Stated in the table's `pg_gibbs` block and pinned by a test that recomputes the kept count from the profile.
    **`sd.load` (1.0) and `re.lkj` (1.5) are laplace rows (#189)**; `n.quad` is NOT one number and deliberately so -- `.TOBS_NQUAD_ROUTES` / `.tobs_n_quad(route)` enumerates the seven marginals it names (`re_aghq` 9, `ms_nmix` 1, `ms_nmix_scalar` 2, `ms_occu_cover` 5, `cover_latent_beta` 15 vs `cover_latent_lognormal` 1 (closed form, needs none), `community_latent` 5). `?tobs` lists them per route instead of stating a single default most routes do not use
  ...
  joint_substrate.R         — shared joint-fit draw / Pareto-k substrate (.tobs_joint_fit, .tobs_joint_draws*) across the occu_cover / cover nested-Laplace joint routes.
    **Outer-grid placement is promoted (#187)**: `.tobs_promote_outer_grid(jf)` lifts `outer_grid_placement` ("fixed"/"auto_recentered"), `_recenter_attempts`, `_prior_added` and `_recenter_declined` (the reason a "fixed" placement stayed fixed, tulpa#293) to the `tobs_fit` top level, spliced everywhere `.tobs_promote_pareto_k` is (occu_cover postprocess, occu_joint, occu_multiscale_cover_joint) plus the cover decode. NOT gated on the grid having MOVED -- a declined recenter is exactly the case worth seeing, and an inert one staying invisible across a whole batch is what filed it. `.tobs_glance_outer_grid(g, x)` adds the two columns in BOTH `glance.tobs_fit()` and `glance.tobs_multiarm_fit()`; the latter is terminal for `cover_fit` (class order cover_fit/tobs_multiarm_fit/tobs_fit) so it never reaches the former.
    **Defaulted grids declare themselves (#186, needs tulpa >= 0.0.132)**: the engine's auto-recenter decides axis PROVENANCE, not field presence -- it moves an axis that is absent, `auto_grid()`-marked, or exactly equal to its own default, and treats anything else as a user pin. tulpaObs writes a grid on every joint fit, so an unmarked default reads as a pin and the rescue goes inert. `.tobs_default_{sigma,alpha,bym2_rho}_grid()` now RETURN `tulpa::auto_grid(...)` (they are the layer that chose the values, and a user grid never passes through them); `.tobs_mark_auto(x, auto)` re-applies the mark wherever a site reshapes a defaulted grid, since `sort()`/`[`/`c()`/`as.numeric()`/`expand.grid()` all drop the attribute -- the cover arm-specific tau translation, the occu_cover pos-arm tau, the copy `alpha_grid`s, the RE `sigma_grid`s, and the EM-path bym2/ar1 pairings. Verified end to end: a defaulted axis reports `declined = "grid_not_collapsed"`, a `control$sigma.grid` one `"axis_pinned"`
