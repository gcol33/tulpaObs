...
`ms_distance` is NOT yet measured either -- its own single-fit cost turned out to be 15-35 min (vs 6-16s for `ms_count`/`ms_occu`), an order of magnitude too expensive for the same screen inside one session, so it is split into gcol33/tulpaObs#166 alongside the `ms_count`/`jsdm` redo above. One seed there (N=40/S=6/Q=2): `factor.starts` 1 -> 887.3s, 8 -> 2087.7s (2.35x), `mag_ratio` 0.9453 vs 0.9595, max `res_cor` diff 0.0199 -- same qualitative pattern the random screens found for the other families, but one seed is not a screen, and #166 now needs to check `ms_distance`'s own test suite for a seed 215-style case too before trusting any screen result.
