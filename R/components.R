...
#'
#' @return A `tulpaOcc_temporal` object
#' @export
...
#'
#' @return A `tulpaOcc_latent` object
#' @export
occu_latent <- function(n_factors, shared = TRUE, constraint = 0,
  ...
  ), class = "tulpaOcc_latent")
}

#' @export
print.tulpaOcc_re <- function(x, ...) {
  cat(sprintf("tulpaOcc RE: %s (%s model)\n", x$type, x$model))
...
#' @export
print.tulpaOcc_temporal <- function(x, ...) {
  cat(sprintf("tulpaOcc temporal: %s\n", x$type))
