...
.dispatch_count <- function(formula, data, family, detection, y, visits,
  ...
  if (!is.null(structs$spatial)) {
    ...
    if (!all(ftypes %in% c("icar", "car_proper", "bym2", "spde"))) {
      stop(sprintf(paste0(
        "count(): an areal field on the count formula supports icar(), ",
        "car_proper(), bym2(), or the continuous-mesh spde(); got '%s'. The ",
        "improper car() and gp() are not yet wired for the count family ",
        "(gcol33/tulpaObs#117)."),
        paste(unique(setdiff(ftypes, c("icar", "car_proper", "bym2", "spde"))),
              collapse = "' / '")), call. = FALSE)
    }
